 ...
 * Fetches all tickets from GoHighLevel with optional filtering
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Transform opportunities to tickets
    let tickets = opportunities.map(transformOpportunityToTicket);

    // Apply filters
    if (department && typeof department === 'string') {
      tickets = tickets.filter((ticket) => ticket.departments[department as keyof typeof ticket.departments]);
    }
